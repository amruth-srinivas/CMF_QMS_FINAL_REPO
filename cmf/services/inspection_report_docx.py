...
def _write_meta_combined_row(table, row_idx: int, fields: tuple[tuple[str, str], ...]) -> None:
    col = 0
    for (label, value), span in zip(fields, _META_COL_SPANS):
        end = col + span - 1
...
def _append_report_sheet_table(doc: Document, payload: Dict[str, Any], sheet: Dict[str, Any]) -> None:
    ...
    max_samples = int(sheet.get("maxSamples") or payload.get("maxSamples") or 3)
    total_cols = int(sheet.get("totalCols") or payload.get("totalCols") or 13)
    table_rows = 3 + 2 + len(rows) + 1 + 4 + 1
    ...
    for fields in meta:
        _write_meta_combined_row(table, r, fields)
        r += 1
    ...
    table.cell(r, col).merge(table.cell(r + 1, col + rem_span - 1))
    ...
    col_widths = _report_col_widths(total_cols, False, max_samples)
...
def build_inspection_report_docx(payload: Dict[str, Any]) -> BytesIO:
    ...
    if sheets:
        ...
        single_sheet = {
            ...
            "maxSamples": payload.get("maxSamples"),
            "totalCols": payload.get("totalCols"),
            "sheet": payload.get("sheet", "1 of 1"),
        ...
        _append_report_banner(doc)
        _append_report_sheet_table(doc, payload, single_sheet)
