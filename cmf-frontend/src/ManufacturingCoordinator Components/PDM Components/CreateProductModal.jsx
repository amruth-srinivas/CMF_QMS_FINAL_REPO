...
const CreateProductModal = ({ 
  ...
  // Fetch vendors when component mounts or when createType becomes 'part'

  useEffect(() => {

    if (createType === 'part' && !hasFetchedVendors.current) {
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      if (createType === 'product') {
        ...
        // Add vendor_id for all parts
        payloadBase.vendor_id = values.vendor_id || null;

        // Add part_detail only for outsource parts, set to null for in-house/standard
        const isOutSource = partTypes.find(t => t.id === values.type_id)?.type_name?.toLowerCase().includes('out');
        if (isOutSource) {
          payloadBase.part_detail = values.part_detail || null;
        } else {
          payloadBase.part_detail = null;
        }
      ...
      >
        ...
        {createType === 'part' && (
          ...
          <>
            ...
            {/* Part Detail Selection for Out-Source Parts */}
            <Form.Item noStyle shouldUpdate={(prev, curr) => prev.type_id !== curr.type_id}>
              {({ getFieldValue }) => {
                ...
                return (
                  <Form.Item
                    name="part_detail"
                    label={<span className="text-xs sm:text-sm">Part Detail</span>}
                    rules={[{ required: true, message: 'Please select part detail!' }]}
                  >
                    <Select placeholder="Select part detail" size="large">
