...
const RawMaterialHistoryTab = ({ materials }) => {
  ...
  const columns = [
    ...
    {
      title: <span style={{ fontWeight: 'bold', color: '#000' }}>Unit Details</span>,
      key: 'unit_details',
      render: (_, record) => {
        if (record.total_length && record.remaining_length !== null) {
          return (
            <div style={{ fontSize: '11px' }}>
              <div>Total: {record.total_length}mm</div>
              <div style={{ color: record.remaining_length > 0 ? '#52c41a' : '#ff4d4f' }}>
                Left: {record.remaining_length}mm
              </div>
            </div>
          );
        }
