...
const ComponentIssuesNotifications = ({ dateRange, onCount }) => {
  ...
  const handleAcknowledge = async (id) => {
    try {
      const response = await fetch(`${config.API_BASE_URL}/component-issues-notifications/${id}/ack`, {
        method: 'PUT',
      });
      if (!response.ok) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text) => <Tag color={text === 'Completed' ? 'green' : text === 'Ongoing' ? 'blue' : 'orange'}>{text || '-'}</Tag>,
      responsive: ['xs', 'sm', 'md', 'lg', 'xl'],
    },
    {
      title: 'Status',
