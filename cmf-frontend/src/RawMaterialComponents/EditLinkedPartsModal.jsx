          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200" style={{ maxHeight: '500px', overflowY: 'auto' }}>
            {loading ? (
              ...
              viewMode === 'tree' ? (
                ...
                <>
                  {filterParts(getAllPartsFlat(orderHierarchyMap[statusEditRecord?.source_order_id || statusEditRecord?.order_id]))
                    .slice((flatPage - 1) * flatPageSize, flatPage * flatPageSize)
                    .map((partNode) => (
                      <div key={partNode.key} className="py-2 border-b border-gray-100">
                        {renderTreeNode(partNode)}
                      </div>
                    ))}
                  {filterParts(getAllPartsFlat(orderHierarchyMap[statusEditRecord?.source_order_id || statusEditRecord?.order_id])).length > flatPageSize && (
                    <div className="flex justify-center mt-4">
                      <Button
                        size="small"
                        onClick={() => setFlatPage(prev => prev + 1)}
                        disabled={flatPage * flatPageSize >= filterParts(getAllPartsFlat(orderHierarchyMap[statusEditRecord?.source_order_id || statusEditRecord?.order_id])).length}
                      >
                        Load More
                      </Button>
                    </div>
